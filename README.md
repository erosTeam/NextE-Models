...
`1 x 3 x 156 x 156` 输入、`1 x 3 x 284 x 284` 输出的 MindSpore Lite FP16 候选。该图在
103、197、237 的 NNRT/NPU 上均完成加载和三次推理，单切片平均约 15 到 19 ms。它仍不发布：
需要先对照现有 ncnn 图确认权重布局和边界语义，再接入完整 Reader 切片、裁剪和合成，最后用
本地实际页面完成无接缝与画质验证。
